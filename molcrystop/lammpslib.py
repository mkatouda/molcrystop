...
"""
    ...
    def set_cell(self, atoms, change=False):
        if change:
            ...
            #self.lmp.reset_box(box_lo, box_hi, xy, xz, yz)
    ...
    def propagate_orig(self, atoms, properties, system_changes, n_steps, dt=None,
                  dt_not_real_time=False, velocity_field=None):
        """"atoms: Atoms object
            Contains positions, unit-cell, ...
        properties: list of str
        ...
        # Extract the forces and energy
        #print('lammps pe:', self.lmp.extract_variable('pe', None, 0))
        self.results['energy'] = convert(
            self.lmp.extract_variable('pe', None, 0),
            "energy", self.units, "ASE"
        )
        ...
        self.results['stress'] = convert(-stress, "pressure", self.units, "ASE")
        #print('stress:', self.resuts['stress'])
        ...
        if self.coord_transform is not None:
            ...
            self.results['forces'] = f.copy()
        #print('foeces:', self.resuts['forces'])
    ...
    def start_lammps(self):
        ...
        with open(self.parameters.lammps_input) as f:
        ...
        # Read lammps input file
        self.lmp.file(self.parameters.lammps_input)

        self.started = True

        self.lmp.command('thermo_style custom ebond eangle edihed eimp epair evdwl ecoul elong etail pe pxx emol')
